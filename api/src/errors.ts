...
export const CommonErrors = {
    InvalidAddress: 'Invalid address. Must be a valid Sui address.',
};
...
export const appErrorHandler = (err: Error, c: Context) => {
  ...
  if (err instanceof ApiAuthError) return c.json({ error: err.message }, 403);

  if (err instanceof CommonError) return c.json({ error: err.message }, 400);
