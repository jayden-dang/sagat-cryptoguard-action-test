...
export function InvitationsPage() {
  ...
  const { data: pendingInvites, isLoading: isPendingLoading, isFetching: isPendingFetching } = useInvitations(false);

  // Only fetch rejected invitations when the user switches to that tab
  const shouldFetchRejected = activeTab === "rejected";
  const { data: rejectedInvites, isLoading: isRejectedLoading, isFetching: isRejectedFetching } = useInvitations(true, shouldFetchRejected);

  const isLoading = activeTab === "pending"
    ? (isPendingLoading && isPendingFetching)
    : (isRejectedLoading && isRejectedFetching);

  const invitations = activeTab === "pending" ? pendingInvites : rejectedInvites;
  ...
  return (
    <div className="max-w-4xl mx-auto mt-8 px-4">
      ...
      {!invitations || invitations.length === 0 ? (
        <EmptyState
          title={activeTab === "pending" ? "No pending invitations" : "No rejected invitations"}
          description={
            activeTab === "pending"
