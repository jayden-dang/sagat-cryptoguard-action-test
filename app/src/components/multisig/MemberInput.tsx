...
import { Trash2, Key, GripVertical, Search } from "lucide-react";
...
export function MemberInput({ member, index, canRemove, onChange, onRemove }: MemberInputProps) {
  ...
  const validation = validatePublicKey(member.publicKey || '');
  const correspondingAddress = validation.isValid ? validation.address : null;
    ...
    >
      ...
      <div className="flex-1 space-y-2">
        ...
        <div>
          ...
          {member.error && (
            <p className="text-sm text-red-500 mt-1">{member.error}</p>
          )}
          {!member.error && member.publicKey && correspondingAddress && (
        ...
        <div className="flex gap-2 items-center">
          ...
          <Input
            type="number"
            value={member.weight}
            onChange={(e) => onChange(member.id, { weight: parseInt(e.target.value) || 1 })}
            className="w-20"
            min={1}
            max={255}
          />
