...
export const createMultisigSchema = z.object({
  ...
  members: z.array(memberSchema).min(2, "At least 2 members required").max(10, "Maximum 10 members allowed"),
  threshold: z.number().min(1),
}).refine((data) => {
  const totalWeight = data.members.reduce((sum, m) => sum + m.weight, 0);
  ...
  const publicKeys = data.members.map(m => m.publicKey).filter(Boolean);
...
export type CreateMultisigForm = z.infer<typeof createMultisigSchema>;