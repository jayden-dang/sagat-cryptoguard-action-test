...
export function useUserMultisigs() {
  ...
  return useQuery({
    queryKey: [QueryKeys.Multisigs, QueryKeys.User, currentAddress],
    queryFn: async (): Promise<MultisigWithMembersForPublicKey[]> => {
      ...
      const connections = await apiClient.getMultisigConnections();

      return (connections[currentAddress?.publicKey] || []).map((m) => {
        const currentMember = m.members.find(
          (member) => member.publicKey === currentAddress?.publicKey,
        )!;
        ...
        return {
          ...
          pendingMembers: m.members.filter(
            (member) => !member.isAccepted && !member.isRejected,
          ).length,
          rejectedMembers: m.members.filter((member) => member.isRejected)
            .length,
        };
      });
    },
